...
class Preprocessing(object):
    ...
    def compute_magnetic(self, n_chunck_o=25):
        self.b_all = np.zeros((0, self.model_resolution), dtype=bool)
        for i in range(np.int(self.num/n_chunck_o)):
            airborne_plane_s = self.airborne_plane[i*n_chunck_o:(i+1)*n_chunck_o]
            dist = self.eu(airborne_plane_s, self.model_grid)
            b = dist < self.range_max
            del dist
            self.b_all = np.vstack((self.b_all, b))
            ...
            vx, vy, vz = self.get_V_mat(Xn, Yn, Zn, airborne_plane_s, n_chunck_o)
    ...
    def get_V_mat(self, Xn, Yn, Zn, rxLoc, n_chunck_o):
        ...
        Ty[:, 2 * nC:] = np.log((dx1 + np.sqrt(dx1 ** 2 + R1)) /
        ...
        Tx[:, 2 * nC:] = np.log((dy1 + np.sqrt(dy1 ** 2 + R1)) /
    ...
    def set_airborne_plane(self, z, ai_resolution, accuracy = True):
        ...
        if accuracy == True:
            ...
            i_0 = 0
            for i_1 in np.arange(25, ai_resolution[0] * ai_resolution[1] + 1 + 25, 25, dtype=int):
